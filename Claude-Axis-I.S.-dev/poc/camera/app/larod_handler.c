...
/**
 * INT8 dequantization parameters
 * These are typical values for YOLOv5n INT8 models from Axis Model Zoo.
 * The actual values should ideally be read from the model's metadata.
 *
 ...
 *
 * INT8 quantized models from Axis Model Zoo output int8_t values that
 * need to be dequantized to float for proper interpretation.
 */
static void parse_yolo_output_int8(LarodContext* ctx, int8_t* output_data,
    ...
    *num_detections = 0;
    ...
    for (int i = 0; i < 25200 && *num_detections < YOLO_MAX_DETECTIONS; i++) {
        ...
        // Dequantize objectness score
        float objectness = ((float)detection_int8[4] - zero_point) * scale;

        if (objectness < ctx->confidence_threshold) continue;

        // Find class with highest score
        int best_class = 0;
        float best_score = ((float)detection_int8[5] - zero_point) * scale;
        for (int c = 1; c < YOLO_NUM_CLASSES; c++) {
            float class_score = ((float)detection_int8[5 + c] - zero_point) * scale;
        ...
        float x_raw = ((float)detection_int8[0] - zero_point) * scale;
        float y_raw = ((float)detection_int8[1] - zero_point) * scale;
        float w_raw = ((float)detection_int8[2] - zero_point) * scale;
        float h_raw = ((float)detection_int8[3] - zero_point) * scale;

        Detection* det = &detections[*num_detections];
        det->class_id = best_class;
        det->confidence = final_confidence;
        // Normalize coordinates to 0-1 (already scaled by dequantization)
        // Note: For some models, bbox coords may already be normalized
        det->x = x_raw / (float)YOLO_INPUT_WIDTH;
        det->y = y_raw / (float)YOLO_INPUT_HEIGHT;
        det->width = w_raw / (float)YOLO_INPUT_WIDTH;
        det->height = h_raw / (float)YOLO_INPUT_HEIGHT;
...
/**
 * Parse YOLO output tensor (FLOAT version - for non-quantized models)
 * Kept for compatibility with float models
 */
...
LarodResult* Larod_Run_Inference(LarodContext* ctx, VdoBuffer* vdo_buffer) {
    ...
    void* output_data = mmap(NULL, output_size, PROT_READ, MAP_SHARED, output_fd, 0);
